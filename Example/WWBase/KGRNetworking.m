...
//        self.jsonToModelTool.acceptDomains = @[Java_Domain,Php_Domain,Gateway_Domain];
    }
    return self;
}
...
//            if (![url hasPrefix:Php_Domain]){
//                url = [NSString stringWithFormat:@"%@%@",Php_Domain,url];
//            }
//        }
    }
    return url;
...
//    [[KGRUserInfoManager sharedUserManager] clearUserInfo];
    Class cls = NSStringFromClass(@"KLoginViewController");
    dispatch_block_t presentBlock = ^{
        UIViewController * loginViewController = [[cls alloc]init];
        ...
        if ([vc isKindOfClass:[cls class]])  return ;
